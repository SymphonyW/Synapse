...
5. 向前端提供 SSE 增量事件流。
6. 转发长期记忆管理接口到 AI Engine Memory RPC。
...
1. config：环境变量读取与默认值
2. api：HTTP 处理器、认证接口、SSE、长期记忆转发
3. domain：任务与认证领域模型
...
1. 认证：注册、登录、登出、当前用户。
2. 任务：创建、查询、取消、批量取消、审批恢复、重放、会话删除。
3. 事件：按 taskID SSE 订阅（支持游标续传与终态 terminal）。
4. 运维：死信查询。
5. 长期记忆：列表、写入、召回、删除。
6. 健康：联动 AI Engine 健康检查。
...
5. 会话清理：按 conversation_id 删除当前用户任务，并级联事件与死信。
6. 可观测性：统一请求日志与任务事件留痕。
