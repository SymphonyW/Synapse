...
| Postgres | [docker-compose.yml](../docker-compose.yml) | 任务、事件、死信、用户、会话持久化；启用向量记忆时也保存长期记忆向量 |
| Redis | [docker-compose.yml](../docker-compose.yml) | Redis Streams 任务投递与 pending reclaim |
...
flowchart TD
    ...
    Runtime --> Memory[FileMemoryStore or PostgresVectorMemoryStore]
...
| 服务 | 是否必须 | 说明 |
|---|---|---|
| Postgres | 否 | 配置 `SYNAPSE_DATABASE_URL` 后启用；必须先完成 Gateway migration |
| Redis | 否 | 配置 `SYNAPSE_REDIS_ADDR` 后启用；不可用时回退内存队列 |
| OpenAI-compatible API | 否 | `mock` provider 不需要；`openai` provider 需要 API Key |
| 文件系统 | 是 | file memory backend 和工具审计日志使用本地文件 |

## 架构优点和限制

| 类型 | 内容 |
|---|---|
| 优点 | Gateway 与模型执行解耦；事件先持久化再 SSE 输出；Gateway schema 已版本化；Agent 工具策略集中在 Runtime；长期记忆和工具 provider 有清晰扩展点 |
| 限制 | 生产安全和发布回滚流程仍待完善；Redis Streams 多 worker 压测和指标导出仍需补强；OpenAPI/MCP provider 目前是扩展壳，真实外部连接待补 |
