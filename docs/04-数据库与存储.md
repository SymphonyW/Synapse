...
```powershell
# 仅用于已经由旧 ensureSchema 创建、且结构检查完全匹配当前 schema 的数据库
.\scripts\dev.ps1 -Task migrate-baseline -Version 4
```

`migrate-baseline` 会先检查关键表、列和索引，匹配后只写入 migration 版本记录；不会捕获 `already exists` 错误来假装迁移成功。生产环境建议使用独立 migration job 和迁移账号，应用账号只保留运行时读写权限。
...
| 操作 | 建议 |
|---|---|
| 新环境初始化 | 先创建数据库，再执行 `migrate-up`，确认 `migrate-status` 为 `current=4 dirty=false` |
| 旧库升级 | 先备份；若结构来自旧 `ensureSchema` 且未记录版本，执行 `migrate-baseline -Version 4` |
