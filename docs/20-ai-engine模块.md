...
| [memory.py](../services/ai-engine-py/app/memory.py) | MemoryStore 契约与 FileMemoryStore |
| [vector_memory.py](../services/ai-engine-py/app/vector_memory.py) | PostgreSQL + pgvector 的向量记忆实现 |
| [embeddings](../services/ai-engine-py/app/embeddings) | 可插拔 embedding provider |
...
| `SYNAPSE_VECTOR_DATABASE_URL` | 空 | 向量后端 PostgreSQL 连接串 |
| `SYNAPSE_VECTOR_EMBEDDING_PROVIDER` | 空 | 当前支持 `openai_compatible` |
| `SYNAPSE_VECTOR_EMBEDDING_MODEL` | 空 | embedding 模型 |
| `SYNAPSE_VECTOR_EMBEDDING_BASE_URL` | 空 | embeddings base URL |
| `SYNAPSE_VECTOR_EMBEDDING_API_KEY` | 空 | embeddings API key |
| `SYNAPSE_VECTOR_EMBEDDING_DIMENSION` | `0` | 向量维度 |
| `SYNAPSE_VECTOR_MEMORY_TOP_K` | `10` | 向量初筛候选数 |
...
| `MemoryWrite` | `AgentRuntimeService.MemoryWrite` | 写入当前 MemoryStore |
| `MemoryRecall` | `AgentRuntimeService.MemoryRecall` | 从当前 MemoryStore 召回 |
...
Runtime 只依赖 MemoryStore 接口。默认 `file` 后端使用 JSON 文件保存每个用户的记忆；配置 `vector` 时，启动装配层会切换到 `PostgresVectorMemoryStore`，Runtime/API 调用方式不变。
...
| 向量后端 | `PostgresVectorMemoryStore` 基于 PostgreSQL + pgvector，使用 embedding provider 生成向量 |
...
flowchart LR
    ...
    E --> V[query vector]
    V --> P[pgvector cosine search]
    P --> R[importance rerank]
    R --> H[MemoryRecallHit]
```

`file` 后端的 `score` 表示关键词命中得分；`vector` 后端的 `score` 表示语义相似度与 importance 的综合得分。向量召回不会伪造关键词，因此 `matched_terms=[]` 是正常结果。
...
2. PostgresVectorMemoryStore 写入、召回、删除、list、用户隔离、空 query、limit、排序稳定性和 embedding 失败。
3. OpenAI-compatible embedding provider 的响应解析与错误处理。
4. 工具协议、标准事件、审批、审计。
5. Local/OpenAPI/MCP provider 注册和执行。
6. Agent regression 指标与用例判定。
...
| file 后端仍是非高并发共享存储 | 需要共享语义召回时切换 `vector` |
| vector 后端仍靠启动时自动建表 | 后续迁移到版本化 migration |
