...
| [features/tasks](../apps/web/src/features/tasks) | 任务 API、审批 payload 工具、任务 hook、任务列表和详情 |
...
| 审批恢复 | paused 任务可 approve 并恢复执行；普通用户可在聊天会话内审批自己触发的高风险工具调用 |
| 聊天高级设置 | 支持 Agent、长期记忆和高风险工具预授权开关；预授权工具通过按钮式多选面板选择，并显示风险等级 |
| 取消 | 支持单任务取消和批量取消 |
| 死信 | 管理员查看死信并重放 |
...
| 死信列表 | 5 秒轮询，仅运维视图 |
| SSE | 选中任务后连接，收到 terminal 后先用事件内 `status` 立即更新本地任务缓存，再关闭连接并有限次回补任务详情 |
...
| `features/chat` | 用户聊天入口、会话列表、Agent 时间线、聊天内审批卡片、工具预授权选择器 |
| `features/ops` | 运维台、死信 |
| `features/tasks` | 任务 API、审批 payload 构造、任务状态与任务视图 |
...
| `POST /v1/tasks/{taskID}/approve` | 审批恢复，owner 或 admin 可调用 |
| `POST /v1/tasks/{taskID}/replay` | 任务重放 |
| `DELETE /v1/conversations/{conversationID}` | 删除会话 |
| `GET /v1/tools/catalog` | 登录用户可读的安全工具目录，用于聊天页工具预授权选择器 |
...
2. 非管理员不能进入工具策略页。
3. 普通用户只能看到自己的任务。
4. 普通用户只能审批自己拥有的 paused 任务；非 owner 审批他人任务返回 403。
5. 管理员可查看全局任务、死信和工具策略，并可审批任意 paused 任务。
6. 后端是权限最终裁决方，前端只做体验层控制。

## 聊天内审批

当当前会话中的任务满足 `status=paused` 且 metadata 含 `agent_required_tool` 时，聊天消息流会在对应 assistant 消息下展示审批请求卡片。卡片展示工具名、风险等级、工具输入、暂停原因和“等待用户审批”状态。

用户点击“批准并继续”后，前端调用 `POST /v1/tasks/{taskID}/approve`。payload 优先使用 `approved_tool_call`，由 `features/tasks/approval.ts` 从任务 metadata 构造；只有旧任务缺少精确输入时才退回 `approved_tools`。审批成功后卡片显示“已批准，任务继续执行中”，任务回到 queued/running，原 SSE 连接或轮询继续带回后续输出。审批失败时错误会显示在卡片和页面错误区域。

审批恢复后的终态同步不依赖缩短 4 秒任务列表轮询：Gateway terminal SSE 事件会携带最终 `status`，`useTaskEvents` 把该状态回调给聊天面板，`useTasks.patchTaskStatus` 立即更新本地任务缓存。随后前端调用 `fetchTask` 做最终一致性刷新；若首次详情请求仍返回 queued/running/paused，会按 100ms、250ms、500ms 做有限回补。该状态缓存同时驱动聊天消息状态 badge、左侧会话最新任务状态和流式光标判定。

## 聊天高级设置

聊天输入区的高级设置保留三个开关：启用 Agent 规划循环、写入长期记忆、预授权高风险工具。预授权开关关闭时，工具选择区隐藏，并且创建任务 metadata 不写入 `approval_granted` 或 `approved_tools`，避免“未开启却提交默认授权”的歧义。

预授权开关开启后，前端调用 `GET /v1/tools/catalog` 获取登录用户可读的工具目录。工具以按钮 / chip 多选展示，按钮内包含工具名、风险等级 badge，以及“需审批”“已禁用”“当前角色不可用”等状态。高风险或 requires_approval 工具优先分组展示，低风险工具只作为兼容旧流程的“其他可选工具”。禁用或当前角色不可用工具会置灰且不可点击；最终是否放行仍由后端 ToolPolicy 和 Runtime 的审批匹配裁决。
