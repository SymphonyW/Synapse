...
通过 `SYNAPSE_AGENT_TOOL_POLICY_JSON` 提供 AI Engine 启动默认策略：
...
provider 默认策略 / 内置默认策略
        ↓
SYNAPSE_AGENT_TOOL_POLICY_JSON
        ↓
Gateway 持久化策略（存在时覆盖，并热更新运行时）
```

当前实现仍保持 env JSON 兼容；但当管理员已经通过控制台保存策略后，持久化策略会成为运行时权威配置。Gateway 保存后会通过 gRPC `ApplyToolPolicy` 热更新 AI Engine，重启 Gateway 时也会把已保存策略再次下发。
...
3. 当前管理中心严格拒绝未知工具名，暂不支持“预埋未来工具”。
4. 若 Gateway 使用内存存储而非 Postgres，管理策略会随进程重启丢失。
