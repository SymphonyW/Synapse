...
> 该文档为历史设计记录，不再作为当前开发计划。
>
> 当前唯一 Roadmap 事实来源是 [docs/roadmap.md](roadmap.md)。README 只保留公开版摘要。
>
> 下方内容保留用于理解设计来路，包含当时对未完成项的判断，可能已与当前代码状态不一致。
...
| 运行时 | 路径 | 当前职责 | 关键依据 |
|---|---|---|---|
| Web Console | [apps/web/src/App.tsx](../apps/web/src/App.tsx) | 登录注册、用户会话视图、运维视图、任务创建、SSE 展示、审批恢复、取消、死信重放、Agent 时间线展示 | [docs/30-web模块.md](30-web模块.md) |
| Gateway | [services/gateway-go](../services/gateway-go) | HTTP API、Cookie Session、任务落库、会话上下文构建、队列投递、SSE、Worker、审批恢复、死信 | [docs/12-gateway-api模块.md](12-gateway-api模块.md), [docs/16-gateway-worker模块.md](16-gateway-worker模块.md) |
| AI Engine | [services/ai-engine-py](../services/ai-engine-py) | gRPC Runtime、mock/OpenAI-compatible provider、Agent loop、工具治理、长期记忆、回归评测 | [docs/20-ai-engine模块.md](20-ai-engine模块.md) |
| Proto 契约 | [proto/synapse/v1/agent.proto](../proto/synapse/v1/agent.proto) | `Health`、`SubmitTask` stream、`MemoryWrite`、`MemoryRecall`、`MemoryDelete`、`MemoryList` | [docs/03-协议与通信.md](03-协议与通信.md) |
...
flowchart TD
    ...
    Queue --> Redis[(Redis Streams optional)]
```
...
| Web -> Gateway SSE | 选中任务后连接 `/v1/tasks/{taskID}/events?last_event_id=...`，前端按 `event_id` 去重并拼接 token | [apps/web/src/App.tsx](../apps/web/src/App.tsx), [docs/03-协议与通信.md](03-协议与通信.md) |
| Gateway -> Store/Queue | `CreateTask` 先写 TaskStore，再 `Enqueue` task id；未配置 Postgres 时使用内存 TaskStore，配置 Postgres 后必须完成 migration；Redis 不可用时可回退内存队列 | [services/gateway-go/internal/api/handlers.go](../services/gateway-go/internal/api/handlers.go), [services/gateway-go/cmd/server/main.go](../services/gateway-go/cmd/server/main.go) |
| Gateway Worker -> AI Engine | Worker `Claim` delivery 后获取数据库 execution lease，调用 `agent.SubmitTask`，终态/暂停/死信后 Ack，崩溃由 Reclaim 恢复 | [services/gateway-go/internal/worker/processor.go](../services/gateway-go/internal/worker/processor.go) |
...
| 召回算法 | 关键词匹配 + importance 加权；无关键词命中时回退最近记忆 | [services/ai-engine-py/app/memory.py](../services/ai-engine-py/app/memory.py), [docs/04-数据库与存储.md](04-数据库与存储.md) |
...
| 能力 | 当前状态判断 | 代码或文档依据 |
|---|---|---|
| MCP 真接入 | 已有 `MCPAdapter` 和 `MCPToolProvider` 壳，但没有绑定具体 MCP SDK、进程管理、鉴权或 transport | [services/ai-engine-py/app/tools/providers.py](../services/ai-engine-py/app/tools/providers.py), [docs/45-功能-Agent工具治理与审批策略.md](45-功能-Agent工具治理与审批策略.md) |
...
| 向量记忆 | `VectorMemoryStore` 方法全部抛 `NotImplementedError`，当前 `FileMemoryStore` 是关键词召回 | [services/ai-engine-py/app/memory.py](../services/ai-engine-py/app/memory.py), [docs/04-数据库与存储.md](04-数据库与存储.md) |
| 记忆管理 UI | Gateway 已有记忆 API，Web 文档明确当前没有完整记忆管理页面；`App.tsx` 只展示 `memory_recall` 事件和 `memory_write_enabled` 开关 | [docs/30-web模块.md](30-web模块.md), [apps/web/src/App.tsx](../apps/web/src/App.tsx) |
| 工具策略管理中心 | 策略现在通过 `SYNAPSE_AGENT_TOOL_POLICY_JSON` 环境变量注入，没有数据库模型和 Web 管理面 | [services/ai-engine-py/app/config.py](../services/ai-engine-py/app/config.py), [docs/45-功能-Agent工具治理与审批策略.md](45-功能-Agent工具治理与审批策略.md) |
| Agent Trace 工作台 | Web 已能从 info 事件构建 Agent 时间线，但没有独立 Trace 查询、过滤、对比和审计工作台 | [apps/web/src/App.tsx](../apps/web/src/App.tsx), [docs/30-web模块.md](30-web模块.md) |
| 真实模型 Benchmark | 回归评测当前用 `model_provider="mock"` 和本地 seed memory，`cases.json` 是稳定 mock 用例；没有真实 provider 成本、延迟、质量基准 | [services/ai-engine-py/app/benchmarks/regression.py](../services/ai-engine-py/app/benchmarks/regression.py), [docs/46-功能-Agent回归评测与门禁.md](46-功能-Agent回归评测与门禁.md) |
| Replay Diff | 当前只有 `/replay` 将非 running 任务重置为 queued 并追加 `replay_requested`；仓库内未发现 diff/compare 实现 | [services/gateway-go/internal/api/handlers.go](../services/gateway-go/internal/api/handlers.go), [docs/43-功能-重试死信与重放.md](43-功能-重试死信与重放.md) |
| Web 工程化拆分 | `App.tsx` 承担类型、API、状态、SSE、聊天、运维、时间线和表单；文档也将“大文件”列为限制 | [apps/web/src/App.tsx](../apps/web/src/App.tsx), [docs/30-web模块.md](30-web模块.md) |
| OpenAPI/Swagger 文档 | 根 README 和 Gateway API 文档都明确当前没有 OpenAPI/Swagger/Apifox/Postman | [README.md](../README.md), [docs/12-gateway-api模块.md](12-gateway-api模块.md) |
